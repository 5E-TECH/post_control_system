...
const CreateLogist = () => {
  ...
  const { createUser } = useUser("logist");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createUser.mutate(newLogist, {
      onSuccess: () => {
        handleSuccess("Logist muvaffaqiyatli yaratildi");
        navigate(buildAdminPath("all-users"));
      },
      onError: (err: any) => {
        handleApiError(err, "Logist yaratishda xatolik yuz berdi");
        setIsLoading(false);
      },
    });
  };
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 sm:mb-6">
        <div className="w-10 h-10 rounded-xl bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center">
          <HeadphonesIcon className="w-5 h-5 text-teal-600 dark:text-teal-400" />
        </div>
        <div>
          <h2 className="text-base sm:text-lg font-bold text-gray-800 dark:text-white">
            Yangi logist yaratish
          </h2>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
            Logist ma'lumotlarini kiriting
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
        ...
        <button
          ...
          className="w-full h-10 sm:h-11 rounded-lg sm:rounded-xl bg-gradient-to-r from-teal-500 to-cyan-600 text-white text-sm sm:text-base font-medium flex items-center justify-center gap-2 shadow-lg shadow-teal-500/25 hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer mt-4 sm:mt-6"
        >
...
export default memo(CreateLogist);
